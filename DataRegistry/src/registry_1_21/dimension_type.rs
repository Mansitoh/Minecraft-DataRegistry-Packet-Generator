...
pub fn generate_default_dimension_type() {
    ...
    for entry in fs::read_dir(data_dir_path).unwrap() {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            let has_skylight = json["has_skylight"].as_bool().unwrap();
            let has_ceiling = json["has_ceiling"].as_bool().unwrap();
            let ultrawarm = json["ultrawarm"].as_bool().unwrap();
            let coordinate_scale = json["coordinate_scale"].as_f64().unwrap();
            let bed_works = json["bed_works"].as_bool().unwrap();
            let respan_anchor_works = json["respawn_anchor_works"].as_bool().unwrap();
            ...
            let piglin_safe = json["piglin_safe"].as_bool().unwrap();
            let has_raids = json["has_raids"].as_bool().unwrap();
