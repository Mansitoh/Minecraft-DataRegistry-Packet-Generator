...
pub fn generate_default_banner_pattern() {
    let registry_type = "banner_pattern";
    ...
    for entry in fs::read_dir(data_dir_path).unwrap() {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            let translation_key = json["translation_key"].as_str().unwrap();
